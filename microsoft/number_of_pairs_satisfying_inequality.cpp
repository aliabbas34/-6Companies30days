class Solution {
/*
Step 1: Rewriting the inequality -> (nums1[i] - nums2[i]) <= (nums1[j] - nums2[j]) + diff

Step 2: Make a new array/vector c which stores nums1[i]-nums2[i].
or c[i] = nums1[i]-nums2[i] for 0 <= i <= n - 1.

Step 3: Problem reduces to find i , j such that 0 <= i < j <= n - 1 and c[i] <= c[j] + diff

Now, This can be solved by using merge sort
    */
public:
    long long int count;
